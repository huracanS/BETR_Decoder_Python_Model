...
class RVFI_Instr:
    ...
    """
    def __init__(self, pc, next_pc, insn, valid=True, trap=False):
        self.pc = pc             # Current instruction execution address
        self.next_pc = next_pc   # Next instruction address  
        self.insn = insn         # Instruction binary encoding
        self.valid = valid       # Whether instruction is valid
        self.trap = trap         # Exception or interrupt valid signal
...
def is_compressed(instr: RVFI_Instr):
    ...
    """
    return (instr.insn & 0x3) != 0x3  # Compressed instruction: last two bits != 11
...
def classify_instr(instr: RVFI_Instr):
    ...
    - BR_NOT_TAKEN : Conditional branch not taken
    - INDIRECT     : Indirect jump (JALR / C.JR / C.JALR)
    - TRAP         : Interrupt/exception
    """
    if instr.trap:
        return 'TRAP'

    opcode = instr.insn & 0x7f      # 32-bit instruction opcode
    instr_16 = instr.insn & 0xffff  # Compressed instruction: lower 16 bits

    # -----------------------------
    # 16-bit Compressed Instruction Detection
    # -----------------------------
    if (instr_16 & 0x3) != 0x3:  # C instruction: last two bits != 11
        c_funct3 = (instr_16 >> 13) & 0b111
        
        # Conditional branches
        if c_funct3 == 0b110 or c_funct3 == 0b111:  # C.BEQZ / C.BNEZ
            return 'BR_TAKEN' if instr.next_pc != instr.pc + 2 else 'BR_NOT_TAKEN'
        
        # Uniferable Jump. Indirect jumps  
        elif c_funct3 == 0b100:  # C.JR / C.JALR
            return 'INDIRECT'
    ...
    # -----------------------------
    branch_opcodes = [0x63]  # Conditional branches
    jalr_opcode = 0x67       # Indirect jumps

    if opcode in branch_opcodes:
        # Check if branch is taken
        if instr.next_pc != instr.pc + 4:
            return 'BR_TAKEN'
        else:
            return 'BR_NOT_TAKEN'
